...
@Processor(ONCHAIN_QUEUES.QUEUE_UPDATE_DROP)
export class UpdateDropProcessor {
  constructor(
  ...
  logger = new Logger(UpdateDropProcessor.name);

  @Process({ name: ONCHAIN_JOBS.JOB_UPDATE_DROP, concurrency: 10 })
  async detectEvent(job: Job<LogsReturnValues>) {
    ...
    try {
      ...
      this.queue.add(ONCHAIN_JOBS.JOB_UPDATE_DROP, event);
    }
  }
}
