...
@Processor(ONCHAIN_QUEUES.QUEUE_ITEM_STAKED)
export class ItemStakedProcessor {
  constructor(
  ...
  logger = new Logger(ItemStakedProcessor.name);

  @Process({ name: ONCHAIN_JOBS.JOB_ITEM_STAKED, concurrency: 10 })
  async detectEvent(job: Job<LogsReturnValues>) {
    ...
    try {
      ...
      this.queue.add(ONCHAIN_JOBS.JOB_ITEM_STAKED, event);
    }
  }
}
