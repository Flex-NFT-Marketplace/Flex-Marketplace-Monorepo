...
@Processor(ONCHAIN_QUEUES.QUEUE_TAKER_ASK)
export class TakerAskProcessor {
  constructor(
  ...
  chain: ChainDocument;
  logger = new Logger(TakerAskProcessor.name);
  ...
  async detectEvent(job: Job<LogsReturnValues>) {
    ...
    try {
      ...
      this.queue.add(ONCHAIN_JOBS.JOB_TAKER_ASK, event);
    }
  }
}
