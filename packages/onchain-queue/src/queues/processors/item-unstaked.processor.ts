...
@Processor(ONCHAIN_QUEUES.QUEUE_ITEM_UNSTAKED)
export class ItemUnstakedProcessor {
  constructor(
  ...
  logger = new Logger(ItemUnstakedProcessor.name);

  @Process({ name: ONCHAIN_JOBS.JOB_ITEM_UNSTAKED, concurrency: 10 })
  async detectEvent(job: Job<LogsReturnValues>) {
    ...
    try {
      ...
      this.queue.add(ONCHAIN_JOBS.JOB_ITEM_UNSTAKED, event);
    }
  }
}
